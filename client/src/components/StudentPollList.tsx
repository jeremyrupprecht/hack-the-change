...
export default function PollList({courseId}: {courseId: string | null}) {
    ...
    const pollList = questions.map((Q: {question: string, id: string, courseId: string, pollId: string}) => {
      return (
        // <Link to={`/polls/answer`}>
          <PollResponseForm question={Q.question} pollId={Q.id} />
        // </Link>
      )
    });
